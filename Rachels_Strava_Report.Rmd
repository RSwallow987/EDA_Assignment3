...
my_data<-saveRDS(files, file = "my_data.rds")
...
elev<-max(b$elevation)-min(b$elevation)

x<-c(0.1,0.2,1,5,5.9)
time<-c(1,2,3,4,5)
df1<-as.data.frame(cbind(time,x))

df1%>%filter(x %% 1 == 0)

splits<-c()
for(i in 1:37){
  aa<-sights_test%>%filter(tot_dist<1)%>%arrange(tot_dist)%>%summarise()
...
km<-seq(from=1,to=round(tot_distance),by=1)#initilize coloumn one of dataframe = the kms 
 ...
 for(i in 1:round(tot_distance){
      new<-sights_test%>%filter(tot_dist<i & tot_dist=>(i-1)) #filter out the data between the two kms relevent = 1km split 
